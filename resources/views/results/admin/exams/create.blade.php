<x-app-layout>
    <x-slot name="title">{{ __('Create New Examination') }}</x-slot>

    <div class="w-full space-y-6">
        <!-- Top Action Bar -->
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-200">
            <div>
                <p class="text-sm text-slate-500">Configure an examination cycle for mark entry and result processing.</p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden w-full">
            <form method="POST" action="{{ route('admin.exams.store') }}" class="p-4 sm:p-6 lg:p-8 space-y-6">
                @csrf

                <div>
                    <x-input-label for="exam_name" :value="__('Exam Name')" class="font-semibold text-slate-700" />
                    <x-text-input id="exam_name" class="block mt-1.5 w-full rounded-lg border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" type="text" name="exam_name" :value="old('exam_name')" placeholder="e.g. Annual Examination 2026" required autofocus />
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                        <x-input-label for="status" :value="__('Initial Lifecycle Status')" class="font-semibold text-slate-700" />
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" class="block mt-1.5 w-full rounded-lg border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm" placeholder="Provide any additional instructions or guidelines for this exam...">{{ old('description') }}</textarea>
                    <x-input-error :messages="$errors->get('description')" class="mt-2" />
                </div>

                <div class="flex flex-col-reverse sm:flex-row items-center justify-end gap-3 pt-5 border-t border-slate-100">
                    <a href="{{ route('admin.exams.index') }}" class="w-full sm:w-auto text-center px-4 py-2.5 text-sm font-medium text-slate-600 hover:text-slate-900">
                        Cancel
                    </a>
