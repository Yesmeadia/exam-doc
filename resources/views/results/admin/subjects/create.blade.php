<x-app-layout>
    <x-slot name="title">{{ __('Add Subject') }}</x-slot>

    <div class="w-full space-y-6">
        <!-- Top Action Bar -->
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-200">
            <div>
                <p class="text-sm text-slate-500">Create a curriculum subject and map it to applicable school classes.</p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden w-full">
            <form method="POST" action="{{ route('admin.subjects.store') }}" class="p-4 sm:p-6 lg:p-8 space-y-6">
                @csrf

                <div>
                    <x-input-label for="name" :value="__('Subject Name (e.g. Mathematics)')" class="font-semibold text-slate-700" />
                    <x-text-input id="name" class="block mt-1.5 w-full rounded-lg border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" type="text" name="name" :value="old('name')" required autofocus placeholder="Mathematics" />
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="maximum_marks" class="block mt-1.5 w-full rounded-lg border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" type="number" step="0.5" name="maximum_marks" :value="old('maximum_marks', 100)" required />
                        <x-input-error :messages="$errors->get('maximum_marks')" class="mt-2" />
                        <p class="text-[11px] text-slate-400 mt-1">Configurable per subject.</p>
                    </div>

                    <div>
                        <x-input-label for="pass_marks" :value="__('Pass Marks')" class="font-semibold text-slate-700" />
                        <x-text-input id="pass_marks" class="block mt-1.5 w-full rounded-lg border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" type="number" step="0.5" name="pass_marks" :value="old('pass_marks', 33)" required />
                        <x-input-error :messages="$errors->get('pass_marks')" class="mt-2" />
                    </div>
                </div>

                <div>
                    <x-input-label :value="__('Map to Classes (Select all that apply)')" class="font-semibold text-slate-700 mb-2" />
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 bg-slate-50 p-4 rounded-xl border border-slate-200">
                        @foreach($classes as $class)
                            <label class="inline-flex items-center text-sm text-slate-700 cursor-pointer">
                                <input type="checkbox" name="class_ids[]" value="{{ $class->id }}" class="rounded border-slate-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
